...
//        System.out.println(((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest().getHeader("Authorization"));
    ...
    public ResponseEntity<?> getOneAccount(@PathVariable("accountId") String accountId) {
        // On vérifie que le compte connecté est bien celui demandé en paramètre => accountId
        if (accountResource.findByPassportNumberEqualsIgnoreCase(currentUsername()).get().getId().equals(accountId) == false) {
            String errorMessage = "{\"message\":\"Request not processed, reason is : accountid provided does not match with account id connected\"}";
            return ResponseEntity.badRequest().body(errorMessage);
        }
        return Optional.ofNullable(accountResource.findById(accountId)).filter(Optional::isPresent)
